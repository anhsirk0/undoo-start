...
let make = (~page: Shape.Page.t, ~isEditing, ~isVisiting) => {
  ...
  let cards = page.sites->Array.mapWithIndex((site, index) => {
    ...
    <SiteCard site key={Int.toString(site.id)} isEditing updateSite index>
      <div
        ...
        onClick=onDelete
        className={`absolute ${pos} size-8 lg:size-10 xxl:size-12 center rounded-bl-box`}>
  ...
  <React.Fragment>
    <div className="grid grid-cols-12 gap-4 lg:gap-8 xl:gap-10 xxl:gap-12 w-full animate-fade">
      {React.array(cards)}
    </div>
