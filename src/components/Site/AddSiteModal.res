type choosingFrom = Noop | Bookmarks(array<Bookmarks.item>) | Topsites(array<Bookmarks.item>)
...
let make = (~site: option<Shape.Site.t>, ~onClose, ~onSubmit) => {
  ...
  let (choosing, setChoosing) = React.useState(_ => Noop)
  let (bookmarks, setBookmarks) = React.useState(_ => [])
  let (topSites, setTopSites) = React.useState(_ => [])
  ...
  React.useEffect0(() => {
    ...
    Browser.getTopSites()->Promise.then(async arr => setTopSites(_ => arr))->ignore
    None
  })
    ...
    >
      <div className="flex flex-col lg:flex-row gap-4 2xl:gap-8 min-h-0">
        <div className="flex flex-col gap-2 xl:gap-4 w-[45%] shrink-0 grow min-h-0">
          {switch choosing {
          | Topsites(bookmarks) | Bookmarks(bookmarks) =>
            <ChooseFromBookmarks
              bookmarks
              onClose={() => setChoosing(_ => Noop)}
              onSubmit={it => {
                setChoosing(_ => Noop)
                setTimeout(() => {
                  it.url->setUrl
      ...
      <div className="flex flex-row gap-4 mt-4">
        ...
        <div className="grow" />
        {topSites->Array.length > 0
          ? <button
              type_="button"
              onClick={_ => setChoosing(_ => Topsites(topSites))}
              className="btn resp-btn btn-accent btn-outline"
            >
              {"Choose from Top Sites"->React.string}
            </button>
          : React.null}
        {bookmarks->Array.length > 0
          ? <button
              type_="button"
              onClick={_ => setChoosing(_ => Bookmarks(bookmarks))}
              className="btn resp-btn btn-accent btn-outline"
            >
